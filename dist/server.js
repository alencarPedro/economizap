    ...
    try {
        ...
        try {
            ...
            if (fs.existsSync(tablesFilePath)) {
                ...
                const tableStatements = tablesContent.split(';').filter((stmt) => stmt.trim().length > 0);
        ...
        try {
            ...
            if (fs.existsSync(functionsFilePath)) {
                ...
                // Split the SQL content into individual statements
                const functionStatements = functionsContent.split(';').filter((stmt) => stmt.trim().length > 0);
                // Execute each statement
...
async function checkTableUserIdFormat(tableName) {
    try {
...
async function getUserRecentExpenses(userId) {
    try {
...
async function getUserSavingsGoals(userId) {
    try {
        // Get the user data first to get the actual user ID
        const { data: userData, error: userError } = await supabase.from('users').select('id').eq('phone', userId).single();
        if (userError) {
...
async function detectAndProcessExpense(userId, text, userData) {
    ...
    const pattern2 = /^(\d+[.,]?\d*)\s+(?:for|para|em|no|na)\s+([a-zA-ZÀ-ÖØ-öø-ÿ\s]+)\s+(?:in|em|no|na|categoria)\s+([a-zA-ZÀ-ÖØ-öø-ÿ\s]+)$/i;
    // Pattern 3: [description] for [amount] in [category]
    // Example: "uber for 12 in transportation" or "coffee for 5.50 in food"
    const pattern3 = /^([a-zA-ZÀ-ÖØ-öø-ÿ\s]+)\s+(?:for|para|por|de|custa|custou)\s+(\d+[.,]?\d*)\s+(?:in|em|no|na|categoria)\s+([a-zA-ZÀ-ÖØ-öø-ÿ\s]+)$/i;
    ...
    const pattern4 = /^(?:spent|gastei|gasto|paguei|pago)\s+(\d+[.,]?\d*)\s+(?:on|em|no|na|com|para|por)\s+([a-zA-ZÀ-ÖØ-öø-ÿ\s]+)\s+(?:for|em|no|na|categoria)\s+([a-zA-ZÀ-ÖØ-öø-ÿ\s]+)$/i;
    // Try each pattern
...
async function analyzeUserFinancialBehavior(userId) {
    try {
        // Get the user data
        ...
        const daysWithExpenses = new Set(expenses.map((e) => new Date(e.created_at).toDateString())).size;
        const dailyAverage = daysWithExpenses > 0 ? totalExpenses / daysWithExpenses : 0;
        // Check if user is on track with savings goals
        let savingsInsight = '';
        if (goals && goals.length > 0) {
            const activeGoals = goals.filter((goal) => new Date(goal.target_date) > new Date());
            if (activeGoals.length > 0) {
                ...
                if (overallProgress < 50) {
                    savingsInsight = `\n\nVocê está com ${overallProgress.toFixed(1)}% de progresso em suas metas de economia. Considere aumentar seus depósitos mensais para atingir seus objetivos.`;
                }
                else {
                    savingsInsight = `\n\nVocê está com bom progresso (${overallProgress.toFixed(1)}%) em suas metas de economia. Continue assim!`;
                }
            }
        }
        // Generate insights
...
async function getAIResponse(userId, userMessage, userData) {
    try {
        // Make sure we have the user data with the correct ID
        if (!userData || !userData.id) {
            const user = await getUserByPhone(userId);
                ...
                try {
                    ...
                    if (error) {
                        ...
                        if (directError) {
                            ...
                            const sortedHistory = [...directHistory].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
                        ...
                        // Add previous messages to the new thread to maintain context
                        // Sort by created_at in ascending order to add oldest messages first
                        const sortedHistory = [...conversationHistory].sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime());
                        for (const message of sortedHistory) {
                            try {
                                await openai.beta.threads.messages.create(thread.id, {
                                    role: message.role,
...
async function updateUserNameFromContact(userId, contact) {
    try {
...
const commandHandlers = {
    async start(message, user) {
        ...
        return (`👋 Olá ${user.name}! Bem-vindo ao EconomiZap!\n\n` +
    ...
    async gasto(message, user, args) {
        return this.despesa(message, user, args);
    },
    async despesa(message, user, args) {
        // Make sure we have the user data with the correct ID
        if (!user || !user.id) {
            const userData = await getUserByPhone(formatWhatsAppId(message.from));
            if (!userData) {
        ...
        if (args && args.length >= 3) {
            try {
                ...
                return `✅ Despesa "${description}" de R$ ${amount.toFixed(2)} na categoria "${category}" adicionada com sucesso!`;
            }
            catch (error) {
